...
// 1. CPHI
const rawCphi = parseCSV(fs.readFileSync('src/data/raw/cphi_materials.csv', 'utf8'));
const cphiData = rawCphi.map((r, i) => ({
  id: `cphi-${i + 1}`,
...
// 2. PROJECT MATERIALS
const rawProj = parseCSV(fs.readFileSync('src/data/raw/project_materials.csv', 'utf8'));
const projectSavingsData = rawProj.map((r, i) => ({
  id: `proj-${i + 1}`,
...
// 3. COMMERCIAL PO
const rawComm = parseCSV(fs.readFileSync('src/data/raw/commercial_po.csv', 'utf8'));
const commercialPoData = rawComm.map((r, i) => ({
  id: `comm-${i + 1}`,
...
// 4. HISTORIC
const rawHist = parseCSV(fs.readFileSync('src/data/raw/historic_commercially_ordered.csv', 'utf8'));
const historicData = rawHist.map((r, i) => ({
  id: `hist-${i + 1}`,
  year: r['Year'] || '',
  materialCode: r['Material Code'] || '',
  company: r['Company'] || 'LAB',
...
// 5. MATURITY (NON-COMMERCIALIZED)
const rawNonComm = parseCSV(fs.readFileSync('src/data/raw/non_commercialized_maturity.csv', 'utf8'));
const maturityData = rawNonComm.map((r, i) => ({
  id: `mat-${i + 1}`,
...
// 6. UNDER DEVELOPMENT
const rawUnderDev = parseCSV(fs.readFileSync('src/data/raw/under_development_pipeline.csv', 'utf8'));
const underDevData = rawUnderDev.map((r, i) => {
...
// Write to files
const projectAndCphiTs = `import { CphiSavingRecord, HistoricVarianceRecord, ProjectCommercialSavingRecord } from '../types';

export const INITIAL_CPHI_DATA: CphiSavingRecord[] = ${JSON.stringify(cphiData, null, 2)};

export const INITIAL_PROJECT_SAVINGS_DATA: ProjectCommercialSavingRecord[] = ${JSON.stringify(projectSavingsData, null, 2)};

export const INITIAL_COMMERCIAL_PO_DATA: ProjectCommercialSavingRecord[] = ${JSON.stringify(commercialPoData, null, 2)};

export const INITIAL_HISTORIC_DATA: HistoricVarianceRecord[] = ${JSON.stringify(historicData, null, 2)};
`;
fs.writeFileSync('src/data/projectAndCphiData.ts', projectAndCphiTs);

const maturityTs = `import { MaturityCommercializedRecord } from '../types';

export const INITIAL_MATURITY_DATA: MaturityCommercializedRecord[] = ${JSON.stringify(maturityData, null, 2)};
`;
fs.writeFileSync('src/data/maturityData.ts', maturityTs);

const underDevTs = `import { AlternateUnderDevRecord } from '../types';

export const INITIAL_UNDER_DEV_DATA: AlternateUnderDevRecord[] = ${JSON.stringify(underDevData, null, 2)};
`;
fs.writeFileSync('src/data/underDevData.ts', underDevTs);

console.log('Successfully generated TypeScript datasets from CSVs!');
